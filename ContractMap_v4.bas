Attribute VB_Name = "ContractMap_v4"
Option Explicit
...
Public Function IsShortDatedContract(ByVal token As String) As Boolean
    Call InitPackTable
    Dim i As Long: token = UCase$(token)
    For i = 1 To MAP_COUNT
        If PackTable(i).code = token Then IsShortDatedContract = True: Exit Function
    Next i
End Function
...
Public Function PackCodeFromShortDated(ByVal token As String) As String
    ...
    Dim i As Long: token = UCase$(token)
    For i = 1 To MAP_COUNT
        If PackTable(i).code = token Then PackCodeFromShortDated = PackTable(i).Pack: Exit Function
    Next i
End Function
...
Public Function PackOffsetFromShortDated(ByVal token As String) As Long
    ...
    Dim i As Long: token = UCase$(token)
    For i = 1 To MAP_COUNT
        If PackTable(i).code = token Then PackOffsetFromShortDated = PackTable(i).offset: Exit Function
    Next i
End Function
